...
// GetRawValue calculates the noise at a given coordinate
func (osg *OpenSimplexGenerator2D) GetRawValue(x float64, y float64) float64 {
...
func (osg *OpenSimplexGenerator2D) Get(x float64, y float64) (v float64) {
  curPersistence := 1.0

  x *= osg.Frequency
  y *= osg.Frequency

  for o := 0; o < osg.Octaves; o++ {
    signal := osg.GetRawValue(x, y)
    v += signal * curPersistence

    x *= osg.Lacunarity
    y *= osg.Lacunarity
    curPersistence *= osg.Persistence
  }
